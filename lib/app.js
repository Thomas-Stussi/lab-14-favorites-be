...
//fetch the spells with a matching index
app.get('/search', async(req, res) => {
  const spellsData = await request.get(`https://api.open5e.com/spells/?search=${req.query.searchQuery}`);

  res.json(spellsData.body);
});
...
app.post('/api/favorites', async(req, res) => {
  ...
  try {
    ...
    VALUES ($1, $2, $3, $4)
    RETURNING *
    `, [req.body.name, req.body.level, req.body.description, req.userId]);
